...
// Causeway north from Batıdere to Fenbridge, then spokes to each hub.
...
export const ZONE2_ROADS: { x: number; z: number }[][] = [
  [{ x: 0, z: 80 }, { x: 0, z: 180 }, { x: -8, z: 240 }, { x: 0, z: 300 }],         // Batıdere -> Fenbridge
...
export const ZONE2_QUESTS: Record<string, QuestDef> = {
  ...
  q_prowler_pelts: {
    ...
    text: "O geçit yolunun her kalası, yağlanmış avcı derisine sarılı kazıkların üstünde durur — çürümenin kemiremediği tek şey budur. Stoğum bitti ve güney açıklıkları şimdiden batıyor. Bana 8 sağlam post getir $N, yoksa hepimiz Batıdere'ye yüzerek gideceğiz.",
  ...
  q_fen_supplies: {
    ...
    giverNpcId: 'provisioner_hale', turnInNpcId: 'provisioner_hale',
    text: "Üç gün önce Batıdere'den çıkan bir kervan sise girdi ve kapı çanını bir daha çalmadı. Enkaz geçit yolu boyunca saçılmış — sandıklar, fıçılar, ne varsa, yavaş yavaş batıyor. Bataklık işini bitirmeden 5 yük mal kurtar.",
  ...
  q_idols: {
    ...
    completionText: "Mezarçağıran işi — Morthen'den de eski, benden de. Tarikat Batıdere'de doğmamış $N. Burada doğmuş ve göl sırlarını saklıyormuş.",
